...
var playerRoutes = FlowRouter.group({
  ...
  // triggersEnter can also be [AccountsTemplates.ensureSignedIn]
})

// Added in attempt to get redirect on logout, but doesn't seem to work
playerRoutes.route('/logout', {
  name: 'logout',
  action: () => {
    Meteor.logout()
    FlowRouter.go('/')
...
playerRoutes.route('/profileEdit', {
  name: 'profileEdit',
  action: () => {
    BlazeLayout.render('galaxyMain', {content: 'profileEdit'})
  }
})
...
adminRoutes.route('/player-list', {
  action: function () {
    BlazeLayout.render('galaxyMain', {content: 'player_list'})
  }
})
