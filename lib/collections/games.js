    ...
    ageDays: {type: Number, label: "Game Age", optional: true, autoValue: function () {
        if (this.isInsert) {
            ...
            if (this.siblingField("startDate").isSet && this.siblingField("startDate").value != null) {
                console.log("Calculating new age in days from startDate: " + Math.round((new Date() - new Date(this.siblingField("startDate").value)) / (1000 * 60 * 60 * 24)));
                return Math.round((new Date() - new Date(this.siblingField("startDate").value)) / (1000 * 60 * 60 * 24));
            }
        }
    }}
});
